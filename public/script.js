...
function applyAdminData(d) {
    ...
    if (d.hero) {
        const h = d.hero;
        const greetEl = document.querySelector('.h1-line:first-child .h1-word');
        if (greetEl && h.greeting_zh) { greetEl.textContent = h.greeting_zh; greetEl.setAttribute('data-lang-zh', h.greeting_zh); greetEl.setAttribute('data-lang-en', h.greeting_en || h.greeting_zh); }
        const prefixEl = document.querySelector('.h1-line:nth-child(2) .h1-word:first-child');
        if (prefixEl && h.prefix_zh) { prefixEl.textContent = h.prefix_zh; prefixEl.setAttribute('data-lang-zh', h.prefix_zh); prefixEl.setAttribute('data-lang-en', h.prefix_en || h.prefix_zh); }
        ...
        if (subEl && h.subtitle_zh) { subEl.textContent = h.subtitle_zh; subEl.setAttribute('data-lang-zh', h.subtitle_zh); subEl.setAttribute('data-lang-en', h.subtitle_en || h.subtitle_zh); }
        const descEl = document.querySelector('.hero-p2');
        if (descEl && h.desc_zh) { descEl.textContent = h.desc_zh; descEl.setAttribute('data-lang-zh', h.desc_zh); descEl.setAttribute('data-lang-en', h.desc_en || h.desc_zh); }
        const scrollEl = document.querySelector('.hero-scroll span');
        if (scrollEl && h.scroll_zh) { scrollEl.textContent = h.scroll_zh; scrollEl.setAttribute('data-lang-zh', h.scroll_zh); scrollEl.setAttribute('data-lang-en', h.scroll_en || h.scroll_zh); }
    }
    ...
    if (d.about) {
        ...
        if (bigP && a.text_zh) { bigP.textContent = a.text_zh; bigP.setAttribute('data-lang-zh', a.text_zh); bigP.setAttribute('data-lang-en', a.text_en || a.text_zh); }
        const mt = document.getElementById('marqueeTrack');
        if (mt && a.marquee_zh) {
            const mzh = a.marquee_zh.endsWith(' ') ? a.marquee_zh : a.marquee_zh + ' ';
            mt.setAttribute('data-lang-zh', mzh);
            mt.setAttribute('data-lang-en', (a.marquee_en || a.marquee_zh).replace(/\s*$/, ' '));
            mt.innerHTML = '<span>' + mzh + '</span><span>' + mzh + '</span>';
        }
    }
    ...
    if (d.projects && d.projects.items) {
        const p = d.projects;
        const secH2 = document.querySelector('#work .sec-h2');
        if (secH2 && p.title_zh) { secH2.textContent = p.title_zh; secH2.setAttribute('data-lang-zh', p.title_zh); secH2.setAttribute('data-lang-en', p.title_en || p.title_zh); }
        const container = document.querySelector('#work .container');
        if (container) {
            // Remove old cards, keep h2
            container.querySelectorAll('.card').forEach(c => c.remove());
            p.items.forEach(item => {
                const isDisabled = item.disabled;
                const hasVideo = item.video && item.video_label_zh;
                const tag = isDisabled ? 'div' : 'a';
                const card = document.createElement(tag);
                if (!isDisabled) { card.href = item.link || '#'; card.target = '_blank'; }
                card.className = 'card reveal' + (isDisabled ? ' card-disabled' : '');
                if (item.video) card.dataset.video = item.video;
                card.innerHTML =
                    '<div class="card-visual ' + (item.visual || 'render-v') + '">' +
                        '<div class="card-num">' + (item.num || '') + '</div>' +
                        '<div class="card-tag">' + (item.tag || '') + '</div>' +
                        '<div class="card-arrow"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 17L17 7M17 7H7M17 7v10"/></svg></div>' +
                    '</div>' +
                    '<div class="card-body">' +
                        '<span class="card-label" data-lang-zh="' + esc(item.label_zh) + '" data-lang-en="' + esc(item.label_en || item.label_zh) + '">' + (item.label_zh || '') + '</span>' +
                        '<h3 data-lang-zh="' + esc(item.title_zh) + '" data-lang-en="' + esc(item.title_en || item.title_zh) + '">' + (item.title_zh || '') + '</h3>' +
                        '<p data-lang-zh="' + esc(item.desc_zh) + '" data-lang-en="' + esc(item.desc_en || item.desc_zh) + '">' + (item.desc_zh || '') + '</p>' +
                        '<div class="card-footer">' +
                            '<span class="card-cta' + (isDisabled ? ' disabled-cta' : '') + '" data-lang-zh="' + esc(item.cta_zh) + '" data-lang-en="' + esc(item.cta_en || item.cta_zh) + '">' + (item.cta_zh || '') + '</span>' +
                            (hasVideo ? '<span class="card-video" onclick="event.preventDefault();event.stopPropagation();window.open(\'' + esc(item.video) + '\',\'_blank\')"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg><span data-lang-zh="' + esc(item.video_label_zh) + '" data-lang-en="' + esc(item.video_label_en || item.video_label_zh) + '">' + (item.video_label_zh || '') + '</span></span>' : '') +
                        '</div>' +
                    '</div>';
    ...
    if (d.skills) {
        const s = d.skills;
        const secH2 = document.querySelector('#skills .sec-h2');
        if (secH2 && s.title_zh) { secH2.textContent = s.title_zh; secH2.setAttribute('data-lang-zh', s.title_zh); secH2.setAttribute('data-lang-en', s.title_en || s.title_zh); }
        const row = document.querySelector('.skill-row');
        if (row && s.categories) {
            row.innerHTML = '';
            s.categories.forEach(cat => {
                const col = document.createElement('div');
                col.className = 'skill-col reveal';
                let inner = '<h3 data-lang-zh="' + esc(cat.name_zh) + '" data-lang-en="' + esc(cat.name_en || cat.name_zh) + '">' + (cat.name_zh || '') + '</h3>';
                if (cat.type === 'bars' && cat.bars) {
                    inner += '<ul>';
                    cat.bars.forEach(bar => {
                        inner += '<li><span>' + esc(bar.name) + '</span><span class="dot-bar"><i style="--w:' + bar.value + '%"></i></span></li>';
                    });
                    inner += '</ul>';
                } else if (cat.chips) {
                    inner += '<div class="chip-wrap">';
                    cat.chips.forEach(chip => {
                        inner += '<span class="chip" data-lang-zh="' + esc(chip.zh) + '" data-lang-en="' + esc(chip.en || chip.zh) + '">' + (chip.zh || '') + '</span>';
                    });
                    inner += '</div>';
                }
                col.innerHTML = inner;
    ...
    if (d.experience) {
        ...
        const secH2 = document.querySelector('#exp .sec-h2');
        if (secH2 && e.title_zh) { secH2.textContent = e.title_zh; secH2.setAttribute('data-lang-zh', e.title_zh); secH2.setAttribute('data-lang-en', e.title_en || e.title_zh); }
        ...
        if (grid) {
            ...
            if (e.education && e.education.length) {
                ...
                block.className = 'exp-block reveal';
                let inner = '<h3 data-lang-zh="教育" data-lang-en="Education">教育</h3>';
                e.education.forEach(edu => {
                    inner += '<div class="exp-card">' +
                        '<div class="ec-top"><strong data-lang-zh="' + esc(edu.school_zh) + '" data-lang-en="' + esc(edu.school_en || edu.school_zh) + '">' + (edu.school_zh || '') + '</strong><span>' + (edu.time || '') + '</span></div>' +
                        '<p class="ec-sub" data-lang-zh="' + esc(edu.sub_zh) + '" data-lang-en="' + esc(edu.sub_en || edu.sub_zh) + '">' + (edu.sub_zh || '') + '</p>' +
                        '<p data-lang-zh="' + esc(edu.desc_zh) + '" data-lang-en="' + esc(edu.desc_en || edu.desc_zh) + '">' + (edu.desc_zh || '') + '</p>' +
                    '</div>';
                });
            ...
            // Work
            if (e.work && e.work.length) {
                const block = document.createElement('div');
                block.className = 'exp-block reveal';
                let inner = '<h3 data-lang-zh="工作" data-lang-en="Work">工作</h3>';
                e.work.forEach(w => {
                    inner += '<div class="exp-card">' +
                        '<div class="ec-top"><strong data-lang-zh="' + esc(w.title_zh) + '" data-lang-en="' + esc(w.title_en || w.title_zh) + '">' + (w.title_zh || '') + '</strong><span>' + (w.time || '') + '</span></div>' +
                        '<p class="ec-sub" data-lang-zh="' + esc(w.company_zh) + '" data-lang-en="' + esc(w.company_en || w.company_zh) + '">' + (w.company_zh || '') + '</p>' +
                        '<p data-lang-zh="' + esc(w.desc_zh) + '" data-lang-en="' + esc(w.desc_en || w.desc_zh) + '">' + (w.desc_zh || '') + '</p>' +
                    '</div>';
                });
                block.innerHTML = inner;
                grid.appendChild(block);
    ...
    if (d.contact) {
        const c = d.contact;
        const bigH2 = document.querySelector('.big-contact');
        if (bigH2 && c.title_zh) { bigH2.textContent = c.title_zh; bigH2.setAttribute('data-lang-zh', c.title_zh); bigH2.setAttribute('data-lang-en', c.title_en || c.title_zh); }
        const sub = document.querySelector('.contact-sub');
        if (sub && c.sub_zh) { sub.textContent = c.sub_zh; sub.setAttribute('data-lang-zh', c.sub_zh); sub.setAttribute('data-lang-en', c.sub_en || c.sub_zh); }
        const linksDiv = document.querySelector('.contact-links');
        if (linksDiv && c.links) {
            linksDiv.innerHTML = '';
            c.links.forEach(link => {
                const a = document.createElement('a');
                a.href = link.href || '#';
                if (link.href && !link.href.startsWith('mailto:') && !link.href.startsWith('tel:')) a.target = '_blank';
                a.textContent = link.text || '';
                linksDiv.appendChild(a);
            });
        }
        const footer = document.querySelector('.footer span');
        if (footer && c.footer) { footer.innerHTML = c.footer; }
    }
    ...
    if (d.profile) {
        ...
        if (avatar && pr.avatar) avatar.src = pr.avatar;
        const username = document.querySelector('.profile-header h2');
        if (username && pr.username) username.textContent = pr.username;
        const navLogo = document.querySelector('.nav-logo');
        if (navLogo && pr.username) navLogo.textContent = pr.username;
        const realname = document.querySelector('.profile-name');
        if (realname && pr.realname_zh) { realname.textContent = pr.realname_zh; realname.setAttribute('data-lang-zh', pr.realname_zh); realname.setAttribute('data-lang-en', pr.realname_en || pr.realname_zh); }
        const tagline = document.querySelector('.profile-tagline');
        if (tagline && pr.tagline_zh) { tagline.textContent = pr.tagline_zh; tagline.setAttribute('data-lang-zh', pr.tagline_zh); tagline.setAttribute('data-lang-en', pr.tagline_en || pr.tagline_zh); }
        ...
        if (details && pr.items) {
            ...
            pr.items.forEach(item => {
                ...
                let valHtml = '';
                if (item.href) {
                    valHtml = '<a href="' + esc(item.href) + '"' + (item.href.startsWith('http') ? ' target="_blank"' : '') + '>' + esc(item.value_zh) + '</a>';
                } else {
                    valHtml = '<span' + (item.isStatus ? ' class="profile-status"' : '') + ' data-lang-zh="' + esc(item.value_zh) + '" data-lang-en="' + esc(item.value_en || item.value_zh) + '">' + esc(item.value_zh) + '</span>';
                }
                div.innerHTML = '<span class="profile-label" data-lang-zh="' + esc(item.label) + '" data-lang-en="' + esc(item.label_en || item.label) + '">' + esc(item.label) + '</span>' + valHtml;
    ...
    // --- Blog section title ---
    if (d.blog) {
        const blogH2 = document.querySelector('#blog .sec-h2');
        if (blogH2 && d.blog.title_zh) { blogH2.textContent = d.blog.title_zh; blogH2.setAttribute('data-lang-zh', d.blog.title_zh); blogH2.setAttribute('data-lang-en', d.blog.title_en || d.blog.title_zh); }
    }
}

function esc(s) { return s ? String(s).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/</g,'&lt;').replace(/>/g,'&gt;') : ''; }

// Load from server API and apply
(async function loadFromServer() {
    try {
        const res = await fetch('/api/data');
        if (res.ok) { const d = await res.json(); applyAdminData(d); }
    } catch(e) { console.log('No server data, using defaults'); }

    // Re-init reveal observers for dynamically inserted elements
    document.querySelectorAll('.reveal').forEach(el => {
        if (!el.classList.contains('show')) revealObs2.observe(el);
    });
    ...
    document.querySelectorAll('.skill-col').forEach((col, i) => { col.style.transitionDelay = (i * 0.1) + 's'; });
    document.querySelectorAll('.exp-block').forEach((bl, i) => { bl.style.transitionDelay = (i * 0.12) + 's'; });
    document.querySelectorAll('[data-split]').forEach(el => initSplitEl(el));
    lightWords();

    // Load custom blog posts from server
    try {
        const res = await fetch('/api/posts');
        if (res.ok) {
            const posts = await res.json();
            Object.assign(blogPosts, posts);
            injectCustomBlogCards();
        }
    } catch(e) {}
})();

// Extra reveal observer for server-loaded elements
const revealObs2 = new IntersectionObserver(entries => {
    entries.forEach(e => {
        if (e.isIntersecting) { e.target.classList.add('show'); e.target.querySelectorAll('.dot-bar i').forEach(bar => bar.classList.add('go')); revealObs2.unobserve(e.target); }
    });
...
function renderBlogPost(postId) {
    ...
    pdfs.forEach(s => {
        ...
        html += '<a class="blog-article-pdf" href="/uploads/' + encodeURI(s.filename) + '" target="_blank">' + label + ' ↗</a>';
    });
...
/* ===== Merge server custom posts ===== */
async function getCustomPosts() {
    try { const res = await fetch('/api/posts'); if (res.ok) return await res.json(); } catch(e) {}
    return {};
}
async function saveCustomPosts(posts) {
    // Save each post individually to server
    for (const [id, post] of Object.entries(posts)) {
        await fetch('/api/posts/' + encodeURIComponent(id), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(post) });
    }
}
...
function injectCustomBlogCards() {
    ...
    if (!container) return;
    container.innerHTML = '';
    // Use blogPosts which already has server data merged in
...
// injectCustomBlogCards called by loadFromServer() after API data is merged
...
async function handlePdfFile(file) {
    ...
    adminProgressFill.style.width = '5%';
    adminProgressText.textContent = '正在上传 PDF...';
    ...
    try {
        ...
        currentPdfFilename = uploadData.filename || file.name;
...
/* ===== Edit existing post ===== */
window.editAdminPost = async function(id) {
    const cp = await getCustomPosts();
    const p = cp[id];
    if (!p) return;
...
adminDeleteBtn.addEventListener('click', async () => {
    ...
    await fetch('/api/posts/' + encodeURIComponent(editingId), { method: 'DELETE' });
